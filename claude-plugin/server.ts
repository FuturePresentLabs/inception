...
const logger = {
  error: (...args: any[]) => process.stderr.write(`inception channel: ${args.map(a => typeof a === 'object' ? JSON.stringify(a) : String(a)).join(' ')}\n`),
  info: (...args: any[]) => process.stderr.write(`inception channel: ${args.map(a => typeof a === 'object' ? JSON.stringify(a) : String(a)).join(' ')}\n`),
  debug: (...args: any[]) => process.stderr.write(`inception channel: ${args.map(a => typeof a === 'object' ? JSON.stringify(a) : String(a)).join(' ')}\n`),
};
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    ...
    case "verdict":
      return handleVerdict(args as { request_id: string; decision: "allow" | "deny" });
...
async function handleMetrics(args: { format?: string }) {
  try {
    if (!currentSessionId) {
      return {
        content: [
...
async function handleVerdict(args: { request_id: string; decision: "allow" | "deny" }) {
  try {
    if (!currentSessionId) {
      return {
        content: [
...
// Hook handlers
async function handleHook(event: string, data: any): Promise<void> {
...
async function updateRegistryStatus(updates: {
  ...
  try {
    ...
    if (updates.status) statusUpdate.status = updates.status;
    if (updates.agent_state) statusUpdate.agent_state = updates.agent_state;
    if (updates.progress !== undefined) statusUpdate.progress = updates.progress;
